<template>
  ...
  <modalSmall v-if="showModal">
    <template v-slot:header
      ><h5 class="modal-title">Delete EmployeeExp</h5>
      <button
        type="button"
        class="close"
        aria-label="Close"
    ...
    <template v-slot:body
      ><div class="mb-3">Bạn có chắc muốn xóa EmployeeExp này ?</div>
    </template>
...
export default {
  ...
  data() {
    return {
      showModal: false,
      employeeIdExp: this.employeeExpId,
    };
  },
  setup() {
  ...
  props: ["employeeExpId", "getEmployeeById"],
  methods: {
    handleDelete() {
      this.showModal = true;
    },
    onSubmit() {
      baseReq
        .delete(`employee-experiences/${this.employeeIdExp}`)
        .then((response) => {
